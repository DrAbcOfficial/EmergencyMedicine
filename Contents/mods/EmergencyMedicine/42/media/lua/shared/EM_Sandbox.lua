-- EmergencyMedicine: runtime access to the mod's sandbox options
-- (defined in media/sandbox-options.txt, three tabs: 应急医疗-通用
-- EmergencyMedicineGeneral / 应急医疗-药物 EmergencyMedicineDrugs /
-- 应急医疗-刷新 EmergencyMedicineLoot). Consumers read values AT RUNTIME
...
function EM_Sandbox_Get(name)
	if SandboxVars ~= nil and SandboxVars.EmergencyMedicine ~= nil then
		local value = SandboxVars.EmergencyMedicine[name]
		if value ~= nil then
			return value
		end
	end
